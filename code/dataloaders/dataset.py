    ...
    def __init__(self, base_dir=None, split='train', transform=None, fold="fold1", sup_type="label"):
        ...
        if self.split == 'train':
            self.all_slices = os.listdir(
                self._base_dir + "/Prostate_training_slices")
            self.sample_list = []
            ...
            self.all_volumes = os.listdir(
                self._base_dir + "/Prostate_training_volumes")
            self.sample_list = []
            for ids in test_ids:
                new_data_list = list(filter(lambda x: re.match(
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            h5f = h5py.File(self._base_dir +
                            "/Prostate_training_slices/{}".format(case), 'r')
        else:
            h5f = h5py.File(self._base_dir +
